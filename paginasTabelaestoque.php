...
$sql = "
    SELECT p.*, m.descricao AS descricao_material
    FROM produtos p
    LEFT JOIN materiais m ON TRIM(p.produto) = TRIM(m.codigo)
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $row['descricao'] = $row['descricao_material'] ?: 'Descrição não encontrada';
